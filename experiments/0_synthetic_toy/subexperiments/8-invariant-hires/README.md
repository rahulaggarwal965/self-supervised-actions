...
![decoded action delta — per-code effect vs no-op, amplified 8× (gray = no change)](decoded_action_delta.png)

**The action barely moves the prediction — the core finding.** The plain counterfactual looks identical across codes not because the probe is bad (it isn't — see the reconstruction panel) but because the **code changes the predicted next-latent by only ~1% of the across-sample variation** (code-induced latent std 0.010 vs sample std 0.985; the code-vs-no-op shift is ~6% of the latent norm). The dynamics model predicts the next frame almost entirely from the *current* frame; the action is a tiny, correctly-aimed nudge (it still cuts error ~18% — the no-action gap — but is dwarfed by the static-scene prediction). The **delta figure** (each code's prediction minus the no-op prediction, ×8) isolates that nudge: a small localized change at the agent that *does* differ across codes, while the static distractors stay flat. This is the fundamental obstacle behind NMI 0.36 — not resolution anymore, but that the objective lets the model lean on the predictable current state instead of the action.
